...
function CreateOnlyChoiceQuestion({
  q,
  acQ,
  excerciseId,
  handleCancelQ,
  ...
  const createOptions = (i) => {
    if (answer > i) setAnswer(answer + 1);

    let valueOptions =
      form.getFieldValue("options") || Array(options.length).fill("");

    valueOptions.splice(i + 1, 0, "");

    const newOptions = [...options];
    newOptions.splice(i + 1, 0, { id: null, content: "" });
  ...
  const deleteOptions = (i) => {
    if (answer > i) setAnswer(answer - 1);
    else if (answer === i) setAnswer(null);
...
export default CreateOnlyChoiceQuestion;
